...
// Helper function to populate smart category dropdown based on base unit
async function populateSmartCategoryDropdown(baseUnit, containerId, selectId, currentCategory = '', includeCustom = false) {
  ...
  try {
    ...
    const data = await apiGetCategoriesForBaseUnit(baseUnit)
...
async function saveEditPattern(index) {
  ...
  try {
    ...
    await apiUpdatePattern(index, updatedPattern)
...
async function restoreBackup(event) {
  ...
  try {
    ...
    const result = await apiRestoreBackup(base64)
